...
const Schedule1 = () => {
  ...
  const fetchSchedule = async (myDate, sportIndex) => {
    if (!selectedCentre || !sports[sportIndex]) return; // Ensure a centre and sport are selected
  
    const centreId = selectedCentre._id; // Get the selected centre ID
    const sportName = sports[sportIndex].name; // Get the selected sport name
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/bookings/view`, {
        ...
        date: myDate.toISOString(), // Use ISO string format for the date
      });
      const filteredData = response.data.bookings.filter(item => {
        ...
        return (
          responseDate.getFullYear() === myDate.year() &&
          responseDate.getMonth() === myDate.month() &&
          responseDate.getDate() === myDate.date()
        );
      });
  ...
  const handleDateChange = async (date) => {
    ...
    await fetchSchedule(date, tab); // Fetch schedule for the new date
  };
  ...
  const tabHandler = async (sportIndex) => {
    setTab(sportIndex);
    await fetchSchedule(selectedDate, sportIndex); // Fetch schedule when switching sports
  };
